"""Public OPDS catalog endpoints (token/code in path - no JWT)."""
...
short_router = APIRouter(prefix="/o", tags=["opds"])
...
def _catalog_root(base: str, prefix: str, token: str) -> str:
    return f"{base}{prefix}/{token}"


def _register_opds_routes(r: APIRouter, prefix: str, *, name_prefix: str) -> None:
    """Mount the same OPDS feed under /api/opds/{token} and /o/{code}."""

    @r.get("/{token}", name=f"{name_prefix}-root", operation_id=f"{name_prefix}_root")
    async def opds_root(token: str, db: AsyncSession = Depends(get_db)):
        ...
        root = _catalog_root(base, prefix, token)
    ...
    @r.get("/{token}/shelf", name=f"{name_prefix}-shelf", operation_id=f"{name_prefix}_shelf")
    ...
    @r.get("/{token}/library", name=f"{name_prefix}-library", operation_id=f"{name_prefix}_library")
    ...
    @r.get("/{token}/download/{chapter_id}", name=f"{name_prefix}-download", operation_id=f"{name_prefix}_download")
...
_register_opds_routes(router, "/api/opds", name_prefix="opds")
_register_opds_routes(short_router, "/o", name_prefix="opds-short")
