...
async def run_batched_kavita_scan(*, reason: str = "sweep") -> dict[str, Any]:
    ...
    async with _kavita_scan_lock:
        ...
        try:
            # Keep caltrash/unorganized out of Kavita before every scan.
            try:
                ebook_pipeline.ensure_ebook_kavita_ignores()
            except Exception as ign_err:
                logger.warning("Ebook Sweep .kavitaignore ensure failed: %s", ign_err)
            ...
            repair: dict[str, Any] = {}
            try:
                repair = await ebook_pipeline.repair_incoherent_kavita_ebook_series()
                logger.info("Ebook Sweep post-scan mega-series repair: %s", repair)
            except Exception as repair_err:
                logger.warning("Ebook Sweep mega-series repair failed: %s", repair_err)
                repair = {"error": str(repair_err)[:300]}

            resync: dict[str, Any] = {}
            try:
                from app.services.ebook_quick_review import resync_applied_ebook_overrides_to_kavita

                resync = await resync_applied_ebook_overrides_to_kavita()
                logger.info("Ebook Sweep post-scan override resync: %s", resync)
            except Exception as resync_err:
                logger.warning("Ebook Sweep override resync failed: %s", resync_err)
                resync = {"error": str(resync_err)[:300]}
            return {
                "ok": True,
                "reason": reason,
                "scan": status,
...
async def _run_sweep_worker(job_id: int) -> None:
    try:
        ...
        try:
            ebook_pipeline.ensure_ebook_kavita_ignores()
            repair = await ebook_pipeline.repair_incoherent_kavita_ebook_series()
            logger.info("Ebook Sweep pre-run Kavita repair: %s", repair)
        except Exception as pre_err:
            logger.warning("Ebook Sweep pre-run Kavita repair failed: %s", pre_err)
        options = await library_ingest.ebook_sweep_options()
