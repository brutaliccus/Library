...
4. **Chapter Forge** when an Audible **ASIN** is present **and** a primary `.m4b` exists (after successful convert, or already a single m4b). LibraForge (`POST /api/chaptering/runs`, `backend=audible-chapters`) looks up Audible chapters and writes sidecar/cue data. **Library Site** then embeds those markers into the `.m4b` via ffmpeg stream copy (no re-encode) — upstream LibraForge does not remux the file. **No ASIN → skip**. **No `.m4b` yet (multipart mp3s still waiting on convert) → skip** — never rename/restructure source parts as “chapters”. **Audible missing chapters / embed failure → soft-continue** with existing markers (do not fail the whole book).
...
**Chapters:** ABS reloads chapter markers from the audio file / absMetadata on scan. Library Site embeds Audible chapters into the `.m4b` after LibraForge lookup; editing only sidecars will not stick after a rescan. Forge finalize also pushes Chapter Forge sidecar markers into ABS when present.
