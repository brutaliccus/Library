          ...
          <div className="hidden md:flex mt-4 w-full items-center gap-2">
            {activeChapterId != null && (
              <button
                type="button"
                onClick={() => navigate(`/read/${activeChapterId}`)}
                className="flex-1 min-w-0 h-12 inline-flex items-center justify-center gap-2 px-4 text-base font-semibold rounded-xl bg-amber-600 text-white hover:bg-amber-500 transition-colors"
              >
                <BookOpen size={20} />
                {readingProgress ? "Continue" : "Read"}
              </button>
