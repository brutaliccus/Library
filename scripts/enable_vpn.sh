...
for need in ("bundled-media", "npm", "vpn"):
    # Keep existing non-empty profiles; always ensure vpn.
    pass
if "vpn" not in parts:
    parts.append("vpn")
...
if [[ "$SKIP_REGISTER" -eq 1 ]]; then
  ...
  if docker compose ps --status running --services 2>/dev/null | grep -qx app \
...
for i in $(seq 1 36); do
  ...
  # Fallback: probe via published HTTP proxy from app network
...
c_green "VPN ready. ABB traffic 뿯↽ ${PROXY_URL}"
