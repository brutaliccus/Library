...
if [[ "$SETUP_MODE" == "browser" ]]; then
  ...
  echo "     (debrid, optional Mullvad/gluetun VPN, Hardcover, NPM, VAPID, Audible, …)"
  echo "     VPN can also be enabled later under Admin → Integrations or Health."
  echo ""
...
echo "  - Updates: cd \"$TARGET\" && bash scripts/update_library.sh  (fetch origin/main + rebuild app)"
echo "  - Docs: docs/ubuntu-server-install.md#updating"
echo ""
echo "Stack dir: $TARGET"
