...
def test_persist_buster_is_v4_and_clears_v3():
    main = MAIN.read_text(encoding="utf-8")
    assert "SHELF_PERSIST_KEY" in main
    assert "clearLegacyShelfPersist" in main
    util = UTIL.read_text(encoding="utf-8")
    assert 'SHELF_PERSIST_KEY = "rq-shelf-cache-v4"' in util
    assert "rq-shelf-cache-v3" in util  # legacy clear list
